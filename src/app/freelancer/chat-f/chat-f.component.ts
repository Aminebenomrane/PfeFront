...
export class ChatFComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  sendMessage() {
    if (this.selectedUser) {
      const message: Message = {
        id: 0,
        chatId: this.webSocketService.generateChatId(this.id_user, this.selectedUser.id || 0),
  ...
  private scrollToBottom() {
    if (this.messageContainer && this.messageContainer.last) {
      this.messageContainer.last.nativeElement.scrollTop = this.messageContainer.last.nativeElement.scrollHeight;
    }
  }
